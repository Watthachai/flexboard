...
if (getApps().length === 0) {
  try {
    ...
    if (missingVars.length > 0) {
      ...
      console.warn(
        `⚠️  Firebase features will be disabled. Server will start in degraded mode.`
      );
...
// Initialize Firestore and Auth with null checks
...
// Helper function to get Firebase unavailable error
