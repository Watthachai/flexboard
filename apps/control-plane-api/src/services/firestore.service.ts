...
export class DashboardService extends FirestoreService {
  ...
  static generateDefaultManifest(dashboardId: string, dashboardName?: string) {
    return {
      schemaVersion: "1.0",
  ...
  static async getDashboardManifestConfig(
    ...
    try {
      ...
      if (!doc.exists) {
        // Get dashboard info for default manifest
        ...
        const defaultManifest = this.generateDefaultManifest(
          dashboardId,
          dashboardData?.name || "New Dashboard"
        );

        // Save default manifest to database
        const manifestData = {
        ...
        await configRef.set(manifestData);
        ...
        return {
          ...
          data: {
            id: "manifest",
            ...manifestData,
            createdAt: manifestData.createdAt.toISOString(),
            updatedAt: manifestData.updatedAt.toISOString(),
          },
          timestamp: new Date().toISOString(),
        };
      }
  ...
  static async updateDashboardManifestConfig(
    ...
    try {
      ...
      let updateData: any = {
        data:
          typeof manifestConfig === "string"
            ? JSON.parse(manifestConfig)
            : manifestConfig,
        updatedAt: new Date(),
        updatedBy: userId,
      };
